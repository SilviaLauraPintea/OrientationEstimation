...
void cholesky::solve(cv::Mat b, cv::Mat &x){
	...
	for(unsigned indy=0; indy<this->n; indy++){
		...
		for(int k=indy-1; k>=0; --k){
			sum -= this->covar.at<double>(indy,k)*x.at<double>(k,0);
		}
		x.at<double>(indy,0) = sum/this->covar.at<double>(indy,indy);
	}

	for(int indy=this->n-1; indy>=0; indy--){
		double sum = x.at<double>(indy,0);
		for(int k=indy+1; k<this->n; k++){
			sum -= this->covar.at<double>(k,indy)*x.at<double>(k,0);
		}
		x.at<double>(indy,0) = sum/this->covar.at<double>(indy,indy);
	}
}
