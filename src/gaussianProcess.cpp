...
int main(){
	...
	cv::randu(train,cv::Scalar(0.0),cv::Scalar(10.0));
	...
	for(unsigned i=0; i<100; i++){
		...
		train(cv::Range(i,i+1),cv::Range::all()) = stupid;

		if(i<10){
			cv::Mat stupid2 = test(cv::Range(i,i+1),cv::Range::all());
			cv::add(stupid2, cv::Scalar((100-i)*10.0), stupid2);
		}
	...
	gaussianProcess gp;
	gp.train(train, targets, &gaussianProcess::expCovar, 100.0);
	...
	for(unsigned i=0; i<test.rows; i++){
		...
		std::cout<<"label: "<<i<<" mean:"<<predi.mean<<" variance:"<<\
			predi.variance<<std::endl;
		dummy.mul(result,predi.mean);
		cv::add(result, cv::Scalar(predi.variance), result);
	}
}
